...
class ConvertNativeImgStream {
  ...
  Future<File?> convertImg(
    ...
    final converted = await convertImgToBytes(imgBytes, width, height,
        rotationFix: rotationFix, quality: quality);
    final bytes = converted;
    final path = pathToSave;
    return _saveImageToFile(bytes, path);
  }

  Future<Uint8List?> convertImgToBytes(
      Uint8List imgBytes, int width, int height,
    ...
    final int w = width;
    final int h = height;
    final Uint8List bytes = jpegData;
    final int rotate = rotationFix;
    if (Platform.isAndroid) {
